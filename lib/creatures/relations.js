...
const wounds = require('./relations/wounds');
...
function woundRoute(router, table, route) {
    ...
    router.route('/:id/' + route)
        ...
        .post(async (req, res, next) => {
            await wounds.insert(req, res, next, req.params.id);
        });
...
module.exports.milestones = (router) => {
    ...
    router.post('/:id/' + route, async (req, res, next) => {
        try {
            await relation.insert(req, req.body, 'creature', req.params.id, table, req.body.insert_id);

            let [rows] = await sql('SELECT * FROM milestone_has_loyalty WHERE milestone_id = ?', [req.body.insert_id]);

            for(let i in rows) {
                let query = 'INSERT INTO creature_has_loyalty (creature_id,loyalty_id,wealth_id,milestone_id,name,occupation) VALUES (?,?,?,?,?,?)';
                let array = [req.params.id, rows[i].loyalty_id, rows[i].wealth_id, req.body.insert_id, 'Unknown', rows[i].occupation];
