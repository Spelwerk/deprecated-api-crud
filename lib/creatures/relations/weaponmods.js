...
async function insert(req, res, next, creatureId, weaponId) {
    ...
    try {
        ...
        let query = 'INSERT INTO creature_has_mod (creature_id,weapon_id,weaponmod_id) VALUES (?,?,?) ON DUPLICATE KEY UPDATE weaponmod_id = ?';
...
async function remove(req, res, next, creatureId, weaponId, modId) {
    ...
    try {
        ...
        let query = 'DELETE FROM creature_has_mod WHERE creature_id = ? AND weapon_id = ? AND weaponmod_id = ?';
