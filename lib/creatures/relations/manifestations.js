...
async function attributes(creatureId, manifestationId) {
    try {
        //todo attribute_is_manifestation
        let [rows] = await sql('SELECT * FROM manifestation WHERE id = ?', [manifestationId]);
        if(rows.length === 0) next(new AppError(404, "Manifestation Error", "The manifestation does not have an attribute, which is erroneous"));
...
async function skills(creatureId, manifestationId) {
    try {
        ...
        if(rows.length === 0) next(new AppError(404, "Manifestation Error", "The manifestation does not have a skill, which is erroneous"));
