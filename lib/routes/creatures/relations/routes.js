...
const common = require('../../generic/generic');
const relationsDefaults = require('../../generic/relations/defaults');
const relationsRoutes = require('../../generic/relations/routes');

const augmentations = require('./augmentations');
const backgrounds = require('./backgrounds');
const bionics = require('./bionics');
const loyalties = require('./loyalties');
const manifestations = require('./manifestations');
const milestones = require('./milestones');
const weaponmods = require('./weaponmods');
...
module.exports.armours = (router) => {
    ...
    relationsRoutes.route(router, 'creature', table, query);
};
...
module.exports.attributes = (router) => {
    ...
    const query = 'SELECT ' +
        ...
        'creature_has_attribute.value AS value_original ' +
...
module.exports.backgrounds = (router) => {
    ...
    relationsDefaults.rootGet(router, 'creature', table, query);

    router.post('/:id/' + route, async (req, res, next) => {
        await backgrounds.insert(req, res, next, req.params.id);
    });

    relationsDefaults.itemGet(router, 'creature', table, query);
    relationsDefaults.itemPut(router, 'creature', table);
    relationsDefaults.itemDelete(router, 'creature', table);
};
...
module.exports.bionics = (router) => {
    ...
    relationsDefaults.rootGet(router, 'creature', table, query);
    relationsDefaults.rootPost(router, 'creature', table);
    relationsDefaults.itemGet(router, 'creature', table, query);
    relationsDefaults.itemPut(router, 'creature', table);
    ...
    router.route('/:id/' + route + '/:item/augmentations')
        .get(async (req, res, next) => {
            ...
            await common.select(req, res, next, call, [req.params.id, req.params.item]);
        })
...
module.exports.expertises = (router) => {
    ...
    const query = 'SELECT ' +
        ...
        'expertise.maximum, ' +
        'creature_has_expertise.value AS value_original ' +
...
module.exports.loyalties = (router) => {
    ...
    router.route('/:id/' + route + '/:item')
        .get(async (req, res, next) => {
            ...
            await common.select(req, res, next, call, [req.params.id, req.params.item], true);
        })
...
module.exports.primals = (router) => {
    ...
    const query = 'SELECT ' +
        ...
        'primal.maximum, ' +
        'creature_has_primal.value AS value_original ' +
...
module.exports.skills = (router) => {
    ...
    const query = 'SELECT ' +
        ...
        'skill.description, ' +
        'skill.icon, ' +
        'creature_has_skill.value AS value_original ' +
