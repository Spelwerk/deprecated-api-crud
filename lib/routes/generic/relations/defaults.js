...
const basic = require('../generic');
const common = require('./relations');
...
function rootPost(router, tableName, relationName) {
    ...
    router.post('/:id/' + relationRoute, async (req, res, next) => {
        await common.insert(req, res, next, tableName, req.params.id, relationName, req.body.insert_id);
    });
}
...
function itemPut(router, tableName, relationName) {
    ...
    router.put('/:id/' + relationRoute + '/:item', async (req, res, next) => {
        await common.update(req, res, next, tableName, req.params.id, relationName, req.params.item);
    });
}
...
function itemDelete(router, tableName, relationName) {
    ...
    router.delete('/:id/' + relationRoute + '/:item', async (req, res, next) => {
        await common.remove(req, res, next, tableName, req.params.id, relationName, req.params.item);
    });
}
