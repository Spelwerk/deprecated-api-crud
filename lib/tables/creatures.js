...
module.exports.put = function(user, id, firstName, nickName, middleName, lastName, calculated, manifestation, appearance, age, biography, description, drive, gender, occupation, personality, pride, problem, shame, pointDoctrine, pointExpertise, pointGift, pointImperfection, pointMilestone, pointSkill, corporationId, countryId, identityId, natureId, wealthId, callback) {
    ...
    async.series([
        ...
        function(callback) {
            ...
            query('SELECT attribute_id AS id, value FROM creature_has_attribute WHERE creature_id = ?', [id], function(err, results) {
                if(err) return callback(err);
        ...
        function(callback) {
            ...
            query('SELECT attribute_id AS id, value FROM wealth_has_attribute WHERE wealth_id = ?', [wealthId], function(err, results) {
                if(err) return callback(err);
                ...
                for(var i in results) {
                    ...
                    wealthAttributes.push({id: id, value: value});
                }
        ...
        function(callback) {
            if(!currentWealth) return callback();

            query('SELECT attribute_id AS id, value FROM wealth_has_attribute WHERE wealth_id = ?', [currentWealth], function(err, results) {
                if(err) return callback(err);
                ...
                for(var i in results) {
                    ...
                    currentWealthAttributes.push({id: id, value: value});
                }
        ...
        function(callback) {
            ...
            for(var i in attributes) {
                attributes[i].changed = false;

                // Loop through attributes in new wealth
                if(wealthAttributes.length !== 0) {
                    for(var x in wealthAttributes) {

                        // If IDs are matched, add to attributes list
                        if(attributes[i].id === wealthAttributes[x].id) {
                            attributes[i].value = parseInt(attributes[i].value) + parseInt(wealthAttributes[x].value);
                            attributes[i].changed = true;
                            changed = true;
                        }
                    }
                }
                ...
                if(currentWealthAttributes.length !== 0) {
                    for(var k in currentWealthAttributes) {

                        // If IDs are matched, subtract from attributes list
                        if(attributes[i].id === currentWealthAttributes[k].id) {
                            attributes[i].value = parseInt(attributes[i].value) - parseInt(currentWealthAttributes[k].value);
                            attributes[i].changed = true;
                            changed = true;
                        }
                    }
                ...
                if(attributes[i].changed === true) {
                    sql += '(' + id + ',' + attributes[i].id + ',' + attributes[i].value + '),';
                }
            }
