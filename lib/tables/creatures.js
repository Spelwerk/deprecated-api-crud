...
function combinationId(creatureId, combination, combinationId, callback) {
    creatureId = parseInt(creatureId);
    combinationId = parseInt(combinationId);

    if(!combinationId) return callback();

    query('INSERT INTO creature_is_' + combination + ' (creature_id,' + combination + '_id) VALUES (?,?) ON DUPLICATE KEY UPDATE ' + combination + '_id = VALUES(' + combination + '_id)', [creatureId, combinationId], callback);
}
...
module.exports.put = function(user, id, firstName, nickName, middleName, lastName, calculated, manifestation, appearance, age, biography, description, drive, gender, occupation, personality, pride, problem, shame, pointDoctrine, pointExpertise, pointGift, pointImperfection, pointMilestone, pointSkill, corporationId, countryId, identityId, natureId, wealthId, callback) {
    ...
    async.series([
        ...
        function(callback) {
            combinationId(id, 'corporation', corporationId, callback);
        },
        function(callback) {
            combinationId(id, 'country', countryId, callback);
        },
        function(callback) {
            combinationId(id, 'identity', identityId, callback);
        },
        function(callback) {
            combinationId(id, 'nature', natureId, callback);
        },
        ...
        function(callback) {
            combinationId(id, 'wealth', wealthId, callback);
        }
...
module.exports.assets = function(router) {
    ...
    router.route('/:id/assets/:itemId/value')
        .put(function(req, res, next) {
...
module.exports.attributes = function(router) {
    ...
    router.route('/:id/attributes/:itemId/value')
        .put(function(req, res, next) {
            relation.value(req, res, next, 'creature', req.params.id, table, req.params.itemId, req.body.value);
        });
};
            ...
            async.series([
                ...
                function(callback) {
                    query('SELECT bionic_id FROM creature_has_bionic WHERE creature_id = ? AND bionic_id = ?', [id, bionicId], function(err, results) {
                        ...
                        if(!results[0]) return callback({status: 403, message: 'Bionic not on creature', error: 'The specified bionic is not in the creature list of bionics. Add it before you can add augmentations'});
...
module.exports.doctrines = function(router) {
    ...
    router.route('/:id/doctrines/:itemId/value')
        .put(function(req, res, next) {
            relation.value(req, res, next, 'creature', req.params.id, table, req.params.itemId, req.body.value);
        });
};
...
module.exports.expertises = function(router) {
    ...
    router.route('/:id/expertises/:itemId/value')
        .put(function(req, res, next) {
            relation.value(req, res, next, 'creature', req.params.id, table, req.params.itemId, req.body.value);
        });
};
...
module.exports.protection = function(router) {
    ...
    router.route('/:id/protection/:itemId/value')
        .put(function(req, res, next) {
            relation.value(req, res, next, 'creature', req.params.id, table, req.params.itemId, req.body.value);
        });
...
module.exports.skills = function(router) {
    ...
    router.route('/:id/skills/:itemId/value')
        .put(function(req, res, next) {
            relation.value(req, res, next, 'creature', req.params.id, table, req.params.itemId, req.body.value);
        });
};
            ...
            async.series([
                ...
                function(callback) {
                    query('SELECT first FROM creature_has_species WHERE creature_id = ? AND species_id = ?', [req.params.id, req.params.itemId], function(err, results) {
                        ...
                        if(!results[0].first) return callback({status: 403, message: 'Forbidden', error: 'Removal of the primary species is impossible as it is too important to the character\'s creation and longevity.'});
...
module.exports.weapons = function(router) {
    ...
    router.route('/:id/weapons/:itemId/value')
        .put(function(req, res, next) {
            relation.value(req, res, next, 'creature', req.params.id, table, req.params.itemId, req.body.value);
        });
    ...
    router.route('/:id/weapons/:itemId/mods')
        ...
        .post(function(req, res, next) {
            ...
            async.each([
                ...
                function(callback) {
                    query('SELECT weapon_id FROM creature_has_weapon WHERE creature_id = ? AND weapon_id = ?', [id, weaponId], function(err, results) {
                        if(err) return callback(err);

                        if(!results[0]) return callback({status: 403, message: 'Weapon not on creature', error: 'The specified weapon is not in the creature list of weapons. Add it before you can add mods'});
                ...
                function(callback) {
                    query('SELECT weaponmod_id FROM weapon_has_weaponmod WHERE weapon_id = ? AND weaponmod_id = ?', [weaponId, modId], function(err, results) {
                        ...
                        if(!results[0]) return callback({status: 403, message: 'Mod not associated with weapon', error: 'The specified mod is not associated with that weapon. Did you send the correct insert_id?'});

                        callback();
                    });
                },
                function(callback) {
                    query('INSERT INTO creature_has_weaponmod (creature_id,weapon_id,weaponmod_id) VALUES (?,?,?)', [id, weaponId, modId], callback);
                }
            ], function(err) {
                if(err) return next(err);
