...
function activate(user, id, bionicId, augmentationId, relationArray, boolean, callback) {
    id = parseInt(id);
    ...
    async.series([
        ...
        function(callback) {
            query('SELECT equipped FROM creature_has_augmentation WHERE creature_id = ? AND bionic_id = ? AND augmentation_id = ?', [id, bionicId, augmentationId], function(err, results) {
                if(err) return callback(err);
        ...
        function(callback) {
            ...
            relationValuesEach(user, id, 'augmentation', relationArray, addId, subId, callback);
        },
        function(callback) {
            if((equipped && boolean) || (!equipped && !boolean)) return callback();
            ...
            weaponAdd(user, id, 'augmentation', augmentationId, callback);
        },
        function(callback) {
            if((equipped && boolean) || (!equipped && !boolean)) return callback();
            ...
            weaponRemove(user, id, 'augmentation', augmentationId, callback);
        },
        function(callback) {
            if((equipped && boolean) || (!equipped && !boolean)) return callback();

            query('UPDATE creature_has_augmentation SET equipped = ? WHERE creature_id = ? AND bionic_id = ? AND augmentation_id = ?', [boolean, id, bionicId, augmentationId], callback);
        }
...
function weaponAdd(user, id, tableName, tableId, callback) {
    id = parseInt(id);
    tableId = parseInt(tableId);
    ...
    async.series([
        ...
        function(callback) {
            query('SELECT weapon_id AS id FROM creature_has_weapon WHERE creature_id = ?', [id], function(err, results) {
                if(err) return callback(err);
...
function weaponRemove(user, id, tableName, tableId, callback) {
    id = parseInt(id);
    tableId = parseInt(tableId);
...
function heal(req, res, next, id, tableName, tableId, healed) {
    var creature_has_table = 'creature_has_' + tableName;
