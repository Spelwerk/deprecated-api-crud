...
module.exports.post = function(user, name, description, icon, manifestationId, speciesId, callback) {
    ...
    async.series([
        ...
        function(callback) {
            ...
            query('INSERT INTO background_is_manifestation (background_id,manifestation_id) VALUES (?,?)', [id, manifestationId], callback);
        },
        function(callback) {
            if(!speciesId) return callback();

            query('INSERT INTO background_is_species (background_id,species_id) VALUES (?,?)', [id, speciesId], callback);
        },

        function(callback) {
            query('INSERT INTO user_has_background (user_id,background_id,owner) VALUES (?,?,1)', [user.id, id], callback);
        }
    ], function(err) {
        callback(err, id);
    });
...
/**
 * Updates background with new values
 *
 * @param user Object
 * @param id Integer
 * @param name String
...
module.exports.put = function(user, id, name, description, icon, manifestationId, speciesId, callback) {
    ...
    async.series([
        ...
        function(callback) {
            ...
            query('INSERT INTO background_is_manifestation (background_id,manifestation_id) VALUES (?,?) ON DUPLICATE KEY UPDATE manifestation_id = VALUES(manifestation_id)', [id, manifestationId], callback);
        },
        function(callback) {
            if(!speciesId) return callback();

            query('INSERT INTO background_is_species (background_id,species_id) VALUES (?,?) ON DUPLICATE KEY UPDATE species_id = VALUES(species_id)', [id, speciesId], callback);
        }
    ], function(err) {
        callback(err);
    });
