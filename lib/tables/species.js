...
module.exports.post = function(user, name, description, icon, playable, manifestation, maxAge, multiplyDoctrine, multiplyExpertise, multiplySkill, callback) {
    ...
    async.series([
        ...
        function(callback) {
            query('INSERT INTO user_has_species (user_id,species_id,owner) VALUES (?,?,1)', [user.id, id], callback);
        }
    ], function(err) {
        callback(err, id);
    });
