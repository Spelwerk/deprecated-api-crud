 ...
 * @param callback
 * @returns callback(err, doctrineId)
 */

module.exports = function(user, name, description, icon, expertiseId, manifestationId, effects, callback) {
    ...
    async.series([
        ...
        function(callback) {
            query('INSERT INTO user_has_doctrine (user_id,doctrine_id,owner) VALUES (?,?,1)', [user.id, doctrineId], callback);
        }
    ], function(err) {
        callback(err, doctrineId);
    });
};
