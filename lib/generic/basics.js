...
let elemental = require('../database/elemental');
...
async function select(req, res, next, query, params, single) {
    ...
    try {
        ...
        if(single && length === 0) return next(new AppError(404, "Not Found", "The requested item could not be found in the database."));
