...
async function select(req, res, next, query, params, single) {
    ...
    try {
        ...
        let data = single
            ...
            : {length: rows.length, rows: rows, fields: fields};
...
async function insert(req, res, next, tableName) {
    try {
        let [id] = await elemental.insert(req.user, req.body, tableName);
...
async function update(req, res, next, tableName, tableId) {
    try {
        await elemental.update(req.user, req.body, tableName, tableId);
...
async function remove(req, res, next, tableName, tableId) {
    try {
        await elemental.remove(req.user, tableName, tableId);
