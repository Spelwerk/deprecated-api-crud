...
module.exports.clone = function(req, res, next, tableName, tableId, adminRestriction, userContent, relationTables) {
    tableId = parseInt(tableId);
    relationTables = relationTables || null;
    ...
    async.series([
        function(callback) {
            query('SELECT * FROM ' + tableName + ' WHERE id = ?', [tableId], function(err, results) {
                ...
                var ignored = ['id', 'canon', 'popularity', 'created', 'deleted', 'updated'],
                    select = results[0],
                    values = ' VALUES (';
        ...
        function(callback) {
            if(!relationTables) return callback();
