...
exports.changeValues = function(call, personId, personList, relationList, currentList, callback) {
    ...
    sequel.query(call, null, callback);
};

exports.customDescription = function(req, personId, tableName, tableId, tableCustom, callback) {
    async.series([
        function(callback) {
            auth.ownership(req, false, 'person', personId, callback);
        },
        function(callback) {
            sequel.query('UPDATE person_has_' + tableName + ' SET custom = ? WHERE person_id = ? AND ' + tableName + '_id = ?', [tableCustom, personId, tableId], callback);
        }
    ],function(err) {
        callback(err);
    });
