...
module.exports.changeEquip = function(personId, tableName, tableId, equipValue, callback) {
    ...
    async.series([
        ...
        function(callback) {
            if(alreadyEquipped) return callback();
        ...
        function(callback) {
            ...
            async.each(relationTables, function(relationHasTableName, callback) {
...
module.exports.changeActivate = function(personId, augmentationId, bionicId, activeValue, callback) {
    var person = {},
        change = {};
    ...
    async.series([
        ...
        function(callback) {
            query('SELECT active FROM person_has_augmentation WHERE person_id = ? AND bionic_id = ? AND augmentation_id = ?', [personId, bionicId, augmentationId], function(err, results) {
                ...
                person.active = !!results[0].active;

                callback();
            });
        },
        function(callback) {
            if(person.active) return callback();
        ...
        function(callback) {
            ...
            // Insert the new values into the person
            changeValues('attribute', personId, person.array, change.array, null, callback);
        },
        function(callback) {
            if(person.active) return callback();
            ...
            // Remove the values from the person
            changeValues('attribute', personId, person.array, null, change.array, callback);
        },
        ...
        function(callback) {
            ...
            // Insert the new values into the person
            changeValues('skill', personId, person.array, change.array, null, callback);
        },
        function(callback) {
            if(person.active) return callback();
            ...
            // Remove the values from the person
            changeValues('skill', personId, person.array, null, change.array, callback);
        },
        ...
        function(callback) {
            ...
            query('SELECT weapon_id FROM augmentation WHERE id = ?', [augmentationId], function(err, result) {
                if(err) return callback(err);

                change.weapon = result[0].weapon_id;
        ...
        function(callback) {
            ...
            if(!change.weapon) return callback();

            if(activeValue === 0) return callback();

            query('INSERT INTO person_has_weapon (person_id,weapon_id) VALUES (?,?)', [personId, change.weapon], callback);
        },
        function(callback) {
            if(person.active) return callback();
            ...
            query('DELETE FROM person_has_weapon WHERE person_id = ? AND weapon_id = ?', [personId, change.weapon], callback);
        }
    ],function(err) {
        callback(err);
    });
