...
module.exports = function(req, tableName, tableId, adminRestriction, callback) {
    ...
    query('SELECT owner,edit FROM user_has_' + tableName + ' WHERE user_id = ? AND ' + tableName + '_id = ?', [req.user.id, tableId], function(err, result) {
        if(err) return callback(err);

        req.user.owner = !!result[0].owner;
        req.user.edit = !!result[0].edit;

        if(!req.user.owner && !req.user.edit) return callback({status: 403, message: 'Forbidden', error: 'User is not owner or administrator and may not change this row'});
