 ...
 */
function keep(user, tableName, tableId, callback) {
    if(!user.id) return callback(new UserNotLoggedInError);
 ...
 * @returns callback(err)
 */
function favorite(user, tableName, tableId, favorite, callback) {
    if(!user.id) return callback(new UserNotLoggedInError);
 ...
 * @returns callback(err)
 */
function edit(user, tableName, tableId, userEditId, edit, callback) {
    if(!user.id) return callback(new UserNotLoggedInError);
