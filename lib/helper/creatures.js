...
function post(user, firstName, worldId, speciesId, optional, callback) {
    ...
    let wealthId = parseInt(optional.wealth_id) || null;
    ...
    async.series([
        ...
        function(callback) {
            // Adds weapon from species with weapon_is_species table
            exclusive.addFromCombination(user, id, 'species', speciesId, 'weapon', true, callback);
        },
        function(callback) {
            changeWealth(user, id, wealthId, callback);
        },
        function(callback) {
...
function put(user, id, optional, callback) {
    ...
    let points = {
        ...
        point_primal: optional.pointDoctrine || null,
    ...
    router.route('/:id/backgrounds')
        ...
        .post(function(req, res, next) {
            ...
            async.series([
                ...
                function(callback) {
                    bionic.add(req.user, creatureId, table, backgroundId, callback);
                },
                function(callback) {
...
module.exports.loyalties = function(router) {
    ...
    router.route('/:id/loyalties/:uqId/create')
        .post(function(req, res, next) {
            ...
            let creature = {firstname: null, lastname: null, world_id: null, species_id: speciesId},
                optional = {wealth_id: null, occupation: null},
            ...
            async.series([
                function(callback) {
                    query('SELECT * FROM creature_has_loyalty WHERE id = ?', [uqId], function(err, results) {
                        ...
                        optional.lastName = results[0].name !== null && results[0].name.indexOf(' ') !== -1
                            ? results[0].name.split(' ')[1]
                            : null;

                        optional.wealth_id = parseInt(results[0].wealth_id) || null;
                        optional.occupation = results[0].occupation || null;
                ...
                function(callback) {
                    post(req.user, creature.firstname, creature.world_id, creature.species_id, optional, function(err, id) {
                        if(err) return callback(err);
    ...
    router.route('/:id/milestones')
        ...
        .post(function(req, res, next) {
            ...
            async.series([
                ...
                function(callback) {
                    bionic.add(req.user, req.params.id, table, req.body.insert_id, callback);
                },
                function(callback) {
...
module.exports.weapons = function(router) {
    ...
    router.route('/:id/weapons/:itemId/equip')
        .put(function(req, res, next) {
            equipment.equip(req.user, req.params.id, table, req.params.itemId, {ignoreArray: ['weaponmod']}, function(err) {
                if(err) return next(err);
    ...
    router.route('/:id/weapons/:itemId/unequip')
        .put(function(req, res, next) {
            equipment.unequip(req.user, req.params.id, table, req.params.itemId, {ignoreArray: ['weaponmod']}, function(err) {
                if(err) return next(err);
