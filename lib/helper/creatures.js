...
let schema = require('../../app/initializers/database').getTable('creature');
...
function combinationText(creatureId, combination, combinationText, callback) {
    ...
    if(!combinationText || combinationText !== undefined) return callback();
    ...
    async.series([
        ...
        function(callback) {
            ...
            for(let key in body) {
                ...
                if(schema.fields.indexOf(key) === -1) continue;
        ...
        // Special Data
        function(callback) {
            async.each(specialTables, function(item, next) {
                combinationText(creatureId, item, body[item], next);
            }, function(err) {
                callback(err);
            });
        },
        ...
        // Combinations
        function(callback) {
            async.each(schema.combinations, function(item, next) {
