...
module.exports.augmentations = function(router) {
    let table = 'augmentation',
        ...
        sql = 'SELECT ' +
            ...
            'LEFT JOIN bionic ON bionic.id = creature_has_augmentation.bionic_id';
            ...
            async.series([
                ...
                function(callback) {
                    query('SELECT bionic_id FROM creature_has_bionic WHERE creature_id = ? AND bionic_id = ?', [id, bionicId], function(err, results) {
                        ...
                        if(!results[0]) return callback(new AppError(400, 'Request Error', 'Bionic is not equipped on creature', 'The bionic supplied in the request does not exist on the creature.'));
                ...
                function(callback) {
                    query('SELECT augmentation_id FROM bionic_has_augmentation WHERE bionic_id = ? AND augmentation_id = ?', [bionicId, augmentationId], function(err, results) {
                        ...
                        if(!results[0]) return callback(new AppError(400, 'Request Error', 'Augmentation not associated with bionic', 'The specified augmentation is not associated with that bionic.'));
    ...
    router.route('/:id/augmentations/bionic/:bionicId')
        .get(function(req, res, next) {
            let call = sql + ' WHERE ' +
                'creature_has_' + table + '.creature_id = ? AND ' +
                'creature_has_' + table + '.bionic_id = ?';

            sequel.get(req, res, next, call, [req.params.id, req.params.bionicId]);
        });
