...
const combination = require('../sql/combination');
const ownership = require('../sql/ownership');
const relation = require('../sql/relation');
const unique = require('../sql/unique');
...
// CREATURE HELPER
const additive = require('../creature/additive');
const augmentation = require('../creature/augmentation');
const bionic = require('../creature/bionic');
const equipment = require('../creature/equipment');
const exclusive = require('../creature/exclusive');
const points = require('../creature/points');
const value = require('../creature/value');
...
let schema = getSchema('creature');
let extraData = [];
let withData = schema.tables.withData;

for(let i in withData) {
    let name = 'creature_with_' + withData[i];
...
function parseFloor(number, floor) {
...
function parseRoof(number, roof) {
...
/**
 * Inserts payload into correct table
 * @deprecated
 ...
 */
function setExtraData(creatureId, body, callback) {
    async.each(extraData, function(item, next) {
        let tableName = item.name,
            tableFields = item.fields;

        let sql = 'INSERT INTO ' + tableName  +' (creature_id,',
            values = ' VALUES(?,',
        ...
        for(let key in body) {
            ...
            if(tableFields.indexOf(key) === -1) continue;
 ...
 * @returns {*}
 */
function changeWealth(user, creatureId, wealthId, callback) {
    if(!wealthId) return callback();

    creatureId = parseInt(creatureId);
    wealthId = parseInt(wealthId);

    let currentWealth = null;

    async.series([
        function(callback) {
            query('SELECT wealth_id AS id FROM creature_is_wealth WHERE creature_id = ?', [creatureId], function(err, results) {
        ...
        function(callback) {
            value.update(user, creatureId, 'wealth', 'attribute', wealthId, currentWealth, callback);
        },
        function(callback) {
            combination('creature', creatureId, 'wealth', wealthId, callback);
        }
    ], function(err) {
        callback(err);
    });
...
/**
 * Deletes a relation from creature and unequips the item, removing all relational values
 * @deprecated
 * @param user Object
 * @param creatureId Integer
 ...
 * @returns callback(err)
 */
function removeEquippedRelation(user, creatureId, tableName, tableId, options, callback) {
    async.each([
        function(callback) {
            ownership(user, 'creature', creatureId, callback);
        },
        function(callback) {
            equipment.unequip(user, creatureId, tableName, tableId, options, callback);
        },
        function(callback) {
            query('DELETE FROM creature_has_' + tableName + ' WHERE creature_id = ? AND ' + tableName + '_id = ?', [creatureId, tableId], callback);
        }
    ], function(err) {
        callback(err);
    });
...
/**
 * Adds a relation to creature and then adds values from the relations
 * Also subtracts a point
 * @deprecated
 * @param user Object
 * @param creatureId Integer
 * @param tableName String
 ...
 * @returns callback(err)
 */
function addRelationWithValues(user, creatureId, tableName, tableId, options, callback) {
    creatureId = parseInt(creatureId);
    ...
    async.series([
        ...
        function(callback) {
            query('INSERT INTO creature_has_' + tableName + ' (creature_id,' + tableName + '_id) VALUES (?,?)', [creatureId, tableId], callback);
        },
        function(callback) {
            value.add(user, creatureId, tableName, tableId, options, callback);
        },
        function(callback) {
            points.change(user, creatureId, tableName, -1, callback);
        }
    ], function(err) {
        callback(err);
    });
...
/**
 * Removes a relation from a creature and then removes values from the relations
 * Also adds point
 * @deprecated
 * @param user Object
 * @param creatureId Integer
 ...
 * @returns callback(err)
 */
function removeRelationWithValues(user, creatureId, tableName, tableId, options, callback) {
    creatureId = parseInt(creatureId);
    tableId = parseInt(tableId);
    ...
    async.series([
        ...
        function(callback) {
            value.subtract(user, creatureId, tableName, tableId, options, callback);
        },
        function(callback) {
            points.change(user, creatureId, tableName, 1, callback);
        }
 ...
 */
function post(user, body, callback) {
    if(!user.id) return callback(new UserNotLoggedInError);

    let creatureId,
        wealthId = parseInt(body.wealth_id);

    let epoch = {id: parseInt(body.epoch_id)},
        species = {id: parseInt(body.species_id)},
        world = {};
    ...
    async.series([
        ...
        function(callback) {
            query('SELECT * FROM epoch WHERE id = ?', [epoch.id], function(err, results) {
                console.log(err);
                if(err) return callback(err);
        ...
        function(callback) {
            query('SELECT * FROM species WHERE id = ?', [species.id], function(err, results) {
                ...
                multiplier = parseFloor(species.multiply_points, 1);
        ...
        function(callback) {
            let sql = 'INSERT INTO creature (user_id,',
                values = ' VALUES(?,',
                array = [user.id];
            ...
            query(sql, array, function(err, result) {
                ...
                creatureId = parseInt(result.insertId);
        ...
        function(callback) {
            ...
            body.points_expertise = parseRoof(body.points_expertise, config.points.maximum.expertise);
            body.points_milestone = parseRoof(body.points_milestone, config.points.maximum.milestone);
            body.points_primal = parseRoof(body.points_primal, config.points.maximum.primal);
            body.points_skill = parseRoof(body.points_skill, config.points.maximum.skill);
            body.points_spell = parseRoof(body.points_spell, config.points.maximum.spell);
        ...
        // Special Data
        function(callback) {
            setExtraData(creatureId, body, callback);
        },

        // Combinations
        function(callback) {
            async.each(schema.tables.isOne, function(item, next) {
                if(item === 'wealth') return next();

                let key = item + '_id';

                combination('creature', creatureId, item, body[key], next);
            }, function(err) {
                callback(err);
            });
        },
        ...
        // Wealth
        function(callback) {
            changeWealth(user, creatureId, wealthId, callback);
        },

        // Creating Species
        function(callback) {
            query('INSERT INTO creature_has_species (creature_id,species_id,first) VALUES (?,?,1)', [creatureId, species.id], callback);
        },

        // Adds weapon from species with weapon_is_species
        function(callback) {
            exclusive.addFromCombination(user, creatureId, 'species', species.id, 'weapon', true, callback);
        },

        // Attributes
        function(callback) {
            query('SELECT attribute_id AS id, value FROM world_has_attribute WHERE world_id = ?', [world.id], function(err, results) {
                if(err) return callback(err);

                if(results.length === 0) return callback(new DatabaseRowNotFoundError);

                for(let i in results) {
                    worldAttributes.push({id: parseInt(results[i].id), value: parseInt(results[i].value)});
                }
        ...
        function(callback) {
            query('SELECT attribute_id AS id, value FROM species_has_attribute WHERE species_id = ?', [species.id], function(err, results) {
                if(err) return callback(err);

                if(results.length === 0) return callback();

                for(let i in results) {
                    speciesAttributes.push({id: parseInt(results[i].id), value: parseInt(results[i].value)});
                }
        ...
        function(callback) {
            if(worldAttributes.length === 0 && speciesAttributes.length === 0) return callback();

            attributeArray = worldAttributes;

            if(speciesAttributes.length === 0) return callback();

            for(let i in attributeArray) {
                for(let x in speciesAttributes) {
                    if(attributeArray[i].id !== speciesAttributes[x].id) continue;

                    attributeArray[i].value += speciesAttributes[x].value;
                }
            }
        ...
        function(callback) {
            if(attributeArray.length === 0) return callback(new AppError(500, "No Attributes", "The creature gained no attributes, which is bad!"));

            let sql = 'INSERT INTO creature_has_attribute (creature_id,attribute_id,value) VALUES ';

            // Create the query
            for(let x in attributeArray) {
                sql += '(' + creatureId + ',' + attributeArray[x].id + ',' + attributeArray[x].value + '),';
            }
        ...
        function(callback) {
            query('SELECT id FROM skill WHERE optional = 0', null, function(err, results) {
                ...
                for(let i in results) {
                    skillArray.push(parseInt(results[i].id));
                }
        ...
        function(callback) {
            query('SELECT skill_id AS id FROM epoch_has_skill WHERE epoch_id = ?', [epoch.id], function(err, results) {
                if(err) return callback(err);
        ...
        function(callback) {
            query('SELECT skill_id AS id FROM skill_is_species WHERE species_id = ?', [species.id], function(err, results) {
                if(err) return callback(err);
        ...
        function(callback) {
            if(skillArray.length === 0) return callback(new AppError(500, "No Skills", "The creature gained no skills, which is bad!"));

            let sql = 'INSERT INTO creature_has_skill (creature_id,skill_id) VALUES ';

            for(let i in skillArray) {
                sql += '(' + creatureId + ',' + skillArray[i] + '),';
            }
        ...
        // Experience
        function(callback) {
            let experienceId = config.defaults.attribute.experience,
                maximum = config.points.maximum.experience;

            let multiplied = age * multiplier;

            let experienceValue = multiplied > maximum
                ? maximum
                : multiplied;

            value.change(user, creatureId, 'attribute', experienceId, experienceValue, callback);
        },

        // User
        function(callback) {
            query('INSERT INTO user_has_creature (user_id,creature_id,owner) VALUES (?,?,1)', [user.id, creatureId], callback);
        }

    ], function(err) {
        callback(err, creatureId);
    });
}
 ...
 * @returns callback(err)
 */
function put(user, creatureId, body, callback) {
    if(!user.id) return callback(new UserNotLoggedInError);
...
/** @deprecated */
module.exports.armours = function(router) {
    let table = 'armour',
        sql = 'SELECT ' +
            'armour.id, ' +
            'armour.name, ' +
    ...
    router.route('/:id/armours')
        ...
        .post(function(req, res, next) {
            relations.post(req, res, next, 'creature', req.params.id, table, req.body.insert_id);
        });

    router.route('/:id/armours/:itemId')
        .get(function(req, res, next) {
            let call = sql + ' WHERE ' +
        ...
        .put(function(req, res, next) {
            relations.put(req, res, next, 'creature', req.params.id, table, req.params.itemId, ['equipped']);
        })
        .delete(function(req, res, next) {
            removeEquippedRelation(req.user, req.params.id, table, req.params.itemId, null, function(err) {
                if(err) return next(err);
    ...
    router.route('/:id/armours/:itemId/equip')
        .put(function(req, res, next) {
            equipment.equip(req.user, req.params.id, table, req.params.itemId, null, function(err) {
                if(err) return next(err);
    ...
    router.route('/:id/armours/:itemId/unequip')
        .put(function(req, res, next) {
            equipment.unequip(req.user, req.params.id, table, req.params.itemId, null, function(err) {
                if(err) return next(err);
...
module.exports.assets = function(router) {
    ...
    router.route('/:id/assets/:itemId/equip')
        .put(function(req, res, next) {
            equipment.equip(req.user, req.params.id, table, req.params.itemId, null, function(err) {
                if(err) return next(err);
    ...
    router.route('/:id/assets/:itemId/unequip')
        .put(function(req, res, next) {
            equipment.unequip(req.user, req.params.id, table, req.params.itemId, null, function(err) {
                if(err) return next(err);
...
module.exports.attributes = function(router) {
    ...
    router.route('/:id/attributes/:itemId')
        ...
        .put(function(req, res, next) {
            relations.put(req, res, next, 'creature', req.params.id, table, req.params.itemId);
        })
        .delete(function(req, res, next) {
            relations.remove(req, res, next, 'creature', req.params.id, table, req.params.itemId);
        });
};
...
module.exports.backgrounds = function(router) {
    let table = 'background',
        valueOptions = {ignoreArray: ['armour', 'bionic', 'shield', 'weapon']},
        sql = 'SELECT ' +
            'background.id, ' +
            'background.name, ' +
    ...
    router.route('/:id/backgrounds')
        ...
        .post(function(req, res, next) {
            ...
            async.series([
                ...
                function(callback) {
                    relation.post(req.user, 'creature', creatureId, table, backgroundId, req.body, null, callback);
                },
                function(callback) {
                    value.add(req.user, creatureId, table, backgroundId, valueOptions, callback);
                },
                function(callback) {
                    bionic.addFromRelation(req.user, creatureId, table, backgroundId, callback);
                },
                function(callback) {
                    exclusive.addFromRelation(req.user, creatureId, 'background', backgroundId, 'armour', false, callback);
                },
                function(callback) {
                    exclusive.addFromRelation(req.user, creatureId, 'background', backgroundId, 'shield', false, callback);
                },
                function(callback) {
                    exclusive.addFromRelation(req.user, creatureId, 'background', backgroundId, 'weapon', false, callback);
                },
                function(callback) {
                    points.change(req.user, creatureId, table, -1, callback);
                }
            ], function(err) {
                if(err) return next(err);
    ...
    router.route('/:id/backgrounds/:itemId')
        ...
        .delete(function(req, res, next) {
            let creatureId = parseInt(req.params.id),
                backgroundId = parseInt(req.params.itemId);
            ...
            async.series([
                ...
                function(callback) {
                    value.subtract(req.user, creatureId, table, backgroundId, valueOptions, callback);
                },
                function(callback) {
                    query('DELETE FROM creature_has_' + table + ' WHERE creature_id = ? AND ' + table + '_id = ?', [creatureId, backgroundId], callback);
                },
                function(callback) {
                    points.change(req.user, creatureId, table, 1, callback);
                }
...
module.exports.bionics = function(router) {
    ...
    router.route('/:id/bionics')
        ...
        .post(function(req, res, next) {
            ...
            async.series([
                ...
                function(callback) {
                    relation.post(req.user, 'creature', creatureId, table, bionicId, req.body, null, callback);
                },
                function(callback) {
                    value.add(req.user, creatureId, table, bionicId, {ignoreArray: ['augmentation', 'software']}, callback);
                }
            ], function(err) {
                if(err) return next(err);
    ...
    router.route('/:id/bionics/:itemId')
        ...
        .delete(function(req, res, next) {
            ...
            async.series([
                ...
                function(callback) {
                    query('SELECT augmentation_id AS id FROM creature_has_augmentation WHERE creature_id = ? AND bionic_id = ?', [creatureId, bionicId], function(err, results) {
                        if(err) return callback(err);

                        if(results.length === 0) return callback();

                        for(let i in results) {
                            augmentationArray.push(parseInt(results[i].id));
                        }
                ...
                function(callback) {
                    ...
                    async.each(augmentationArray, function(augmentationId, next) {
                        augmentation.remove(req.user, creatureId, bionicId, augmentationId, next);
                    }, function(err) {
                        callback(err);
                    });
                },
                function(callback) {
                    value.subtract(req.user, creatureId, table, bionicId, {ignoreArray: ['augmentation', 'software']}, callback);
                },
                function(callback) {
                    relation.remove(req.user, 'creature', creatureId, table, bionicId, callback);
                }
            ], function(err) {
                if(err) return next(err);
    ...
    router.route('/:id/bionics/:itemId/augmentations')
        ...
        .post(function(req, res, next) {
            augmentation.add(req.user, req.params.id, req.params.itemId, req.body.insert_id, function(err) {
                if(err) return next(err);
    ...
    router.route('/:id/bionics/:itemId/augmentations/:augmentationId')
        .delete(function(req, res, next) {
            augmentation.remove(req.user, req.params.id, req.body.bionic_id, req.body.insert_id, function(err) {
                if(err) return next(err);
...
module.exports.expertises = function(router) {
    ...
    router.route('/:id/expertises')
        ...
        .post(function(req, res, next) {
            additive.post(req, res, next, req.params.id, table, req.body.insert_id);
        });
    ...
    router.route('/:id/expertises/:itemId')
        ...
        .put(function(req, res, next) {
            additive.put(req, res, next, req.params.id, table, req.params.itemId);
        })
        .delete(function(req, res, next) {
            additive.remove(req, res, next, req.params.id, table, req.params.itemId);
        });
};
...
module.exports.forms = function(router) {
    ...
    router.route('/:id/forms')
        ...
        .post(function(req, res, next) {
            let creatureId = parseInt(req.params.id),
                formId = parseInt(req.body.insert_id);

            async.series([
                function(callback) {
                    relation.post(req.user, 'creature', creatureId, table, formId, req.body, null, callback);
                },
                function(callback) {
                    points.change(req.user, creatureId, table, -1, callback);
                }
    ...
    router.route('/:id/forms/:itemId')
        ...
        .delete(function(req, res, next) {
            let creatureId = parseInt(req.params.id),
                formId = parseInt(req.params.itemId);

            async.series([
                function(callback) {
                    relation.remove(req.user, 'creature', creatureId, table, formId, callback);
                },
                function(callback) {
                    points.change(req.user, creatureId, table, 1, callback);
                }
...
module.exports.gifts = function(router) {
    ...
    router.route('/:id/gifts')
        ...
        .post(function(req, res, next) {
            addRelationWithValues(req.user, req.params.id, table, req.body.insert_id, null, function(err) {
                if(err) return next(err);
    ...
    router.route('/:id/gifts/:itemId')
        ...
        .delete(function(req, res, next) {
            removeRelationWithValues(req.user, req.params.id, table, req.params.itemId, null, function(err) {
                if(err) return next(err);
...
module.exports.languages = function(router) {
    ...
    router.route('/:id/languages')
        ...
        .post(function(req, res, next) {
            let creatureId = parseInt(req.params.id),
                languageId = parseInt(req.body.insert_id);
            ...
            async.series([
                ...
                function(callback) {
                    relation.post(req.user, 'creature', creatureId, table, languageId, req.body, null, callback);
                },
                function(callback) {
                    points.change(req.user, creatureId, table, -1, callback);
                }
    ...
    router.route('/:id/languages/:itemId')
        ...
        .delete(function(req, res, next) {
            let creatureId = parseInt(req.params.id),
                languageId = parseInt(req.params.itemId);
            ...
            async.series([
                ...
                function(callback) {
                    relation.remove(req.user, 'creature', creatureId, table, languageId, callback);
                },
                function(callback) {
                    points.change(req.user, creatureId, table, 1, callback);
                }
...
module.exports.loyalties = function(router) {
    ...
    router.route('/:id/loyalties/:uqId/create')
        .post(function(req, res, next) {
            let creatureId = parseInt(req.params.id),
                uqId = parseInt(req.params.uqId),
                speciesId = parseInt(req.body.species_id),
            ...
            async.series([
                // Build creature information
                function(callback) {
                    query('SELECT * FROM creature_has_loyalty WHERE id = ?', [uqId], function(err, results) {
                        if(err) return callback(err);

                        if(results.length === 0) return callback(new DatabaseRowNotFoundError);

                        relation.loyalty_id = parseInt(results[0].loyalty_id);
                        relation.milestone_id = parseInt(results[0].milestone_id) || null;

                        creature.creaturetype_id = parseInt(req.body.creaturetype_id) || config.defaults.creatureType.avatar;

                        creature.firstname = results[0].name !== null && results[0].name.indexOf(' ') !== -1
                            ? results[0].name.split(' ')[0]
                            : results[0].name;

                        creature.lastname = results[0].name !== null && results[0].name.indexOf(' ') !== -1
                            ? results[0].name.split(' ')[1]
                            : null;

                        creature.wealth_id = parseInt(results[0].wealth_id) || null;
                        creature.occupation = results[0].occupation || null;
                ...
                // Getting epoch
                function(callback) {
                    query('SELECT epoch_id AS id FROM creature WHERE id = ?', [creatureId], function(err, results) {
                        if(err) return callback(err);

                        if(results.length === 0) return callback(new DatabaseRowNotFoundError);

                        creature.epoch_id = parseInt(results[0].id);
                ...
                // Creating creature
                function(callback) {
                    post(req.user, creature, function(err, id) {
                        if(err) return callback(err);
                ...
                // Creating new advanced relation
                function(callback) {
                    query('INSERT INTO creature_has_relation (creature_id,relation_id,loyalty_id,milestone_id) VALUES (?,?,?,?)', [relation.creature_id, relation.relation_id, relation.loyalty_id, relation.milestone_id], callback);
                },

                // Removing old basic relation
                function(callback) {
                    query('DELETE FROM creature_has_loyalty WHERE id = ?', [uqId], callback);
                }
            ], function(err) {
                if(err) return next(err);

                res.status(201).send({id: relation.relation_id});
            });
        });
};
...
module.exports.manifestations = function(router) {
    ...
    router.route('/:id/manifestations')
        ...
        .post(function(req, res, next) {
            ...
            async.series([
                ...
                function(callback) {
                    query('INSERT INTO creature_has_manifestation (creature_id,manifestation_id) VALUES (?,?)', [creatureId, manifestationId], callback);
                },
                function(callback) {
                    query('SELECT attribute_id AS id FROM manifestation WHERE id = ?', [manifestationId], function(err, results) {
                        if(err) return callback(err);

                        if(results.length === 0) return callback(new AppError(404, "Manifestation Error", "The manifestation does not have an attribute, which is erroneous", "An attribute could not be found when looking for it in the table"));
                ...
                function(callback) {
                    query('INSERT INTO creature_has_attribute (creature_id,attribute_id,value) VALUES (?,?,4)', [creatureId, attributeId], callback);
                },
                function(callback) {
                    query('SELECT skill_id AS id FROM skill_is_manifestation WHERE manifestation_id = ?', [manifestationId], function(err, results) {
                        if(err) return callback(err);

                        if(results.length === 0) return callback(new AppError(404, "Manifestation Error", "The manifestation does not have a skill, which is erroneous", "A skill could not be found when looking for it in the table"));
                ...
                function(callback) {
                    ...
                    // Create the query
                    for(let i in skillArray) {
                        sql += '(' + creatureId + ',' + parseInt(skillArray[i].id) + '),';
                    }
                ...
                function(callback) {
                    exclusive.addFromCombination(req.user, creatureId, 'manifestation', manifestationId, 'weapon', true, callback);
                }
            ], function(err) {
                if(err) return next(err);
    ...
    router.route('/:id/manifestations/:itemId')
        ...
        .delete(function(req, res, next) {
            let creatureId = parseInt(req.params.id),
                manifestationId = parseInt(req.params.itemId),
                attributeId,
                skillArray = [];
            ...
            async.series([
                ...
                function(callback) {
                    query('SELECT attribute_id AS id FROM manifestation WHERE id = ?', [manifestationId], function(err, results) {
                        ...
                        attributeId = results[0].id;
                    });
                },
                function(callback) {
                    query('DELETE FROM creature_has_attribute WHERE creature_id = ? AND attribute_id = ?', [creatureId, attributeId], callback);
                },
                function(callback) {
                    query('SELECT skill_id AS id FROM skill_is_manifestation WHERE manifestation_id = ?', [manifestationId], function(err, results) {
                        if(err) return callback(err);
                ...
                function(callback) {
                    let sql = 'DELETE FROM creature_has_skill WHERE creature_id = ? AND (',
                        params = [creatureId];

                    for(let i in skillArray) {
                        sql += 'skill_id = ? OR ';

                        params.push(parseInt(skillArray[i].id));
                    }
                ...
                function(callback) {
                    exclusive.removeFromCombination(req.user, creatureId, 'manifestation', manifestationId, 'weapon', callback);
                }
            ], function(err) {
                if(err) return next(err);
...
module.exports.milestones = function(router) {
    let table = 'milestone',
        valueOptions = {ignoreArray: ['armour', 'bionic', 'loyalty', 'shield', 'weapon']},
        sql = 'SELECT ' +
            'milestone.id, ' +
            'milestone.name, ' +
    ...
    router.route('/:id/milestones')
        ...
        .post(function(req, res, next) {
            ...
            async.series([
                ...
                function(callback) {
                    relation.post(req.user, 'creature', creatureId, table, milestoneId, req.body, null, callback);
                },
                function(callback) {
                    value.add(req.user, creatureId, table, milestoneId, valueOptions, callback);
                },
                function(callback) {
                    bionic.addFromRelation(req.user, req.params.id, table, req.body.insert_id, callback);
                },
                function(callback) {
                    exclusive.addFromRelation(req.user, creatureId, 'milestone', milestoneId, 'armour', false, callback);
                },
                function(callback) {
                    exclusive.addFromRelation(req.user, creatureId, 'milestone', milestoneId, 'shield', false, callback);
                },
                function(callback) {
                    exclusive.addFromRelation(req.user, creatureId, 'milestone', milestoneId, 'weapon', false, callback);
                },
                function(callback) {
                ...
                function(callback) {
                    ...
                    async.each(loyaltyArray, function(object, next) {
                        ...
                        query('INSERT INTO creature_has_loyalty (creature_id,loyalty_id,wealth_id,milestone_id,name,occupation) VALUES (?,?,?,?,?,?)', [creatureId, loyaltyId, wealthId, milestoneId, name, occupation], next);
                    }, function(err) {
                        callback(err);
                    });
                },
    ...
    router.route('/:id/milestones/:itemId')
        ...
        .delete(function(req, res, next) {
            removeRelationWithValues(req.user, req.params.id, table, req.params.itemId, valueOptions, function(err) {
                if(err) return next(err);
...
/** @deprecated */
module.exports.primals = function(router) {
    let table = 'primal',
        sql = 'SELECT ' +
            'primal.id, ' +
            'primal.name, ' +
    ...
    router.route('/:id/primals/:itemId')
        .get(function(req, res, next) {
            let call = sql + ' WHERE ' +
                'creature_has_' + table + '.creature_id = ? AND ' +
                'creature_has_' + table + '.' + table + '_id = ?';
...
/** @deprecated */
module.exports.relations = function(router) {
    let table = 'relation',
        sql = 'SELECT ' +
        'creature.id, ' +
        'creature.firstname, ' +
    ...
    router.route('/:id/relations')
        .get(function(req, res, next) {
            let call = sql + ' WHERE ' +
                'creature_has_relation.creature_id = ?';
    ...
    router.route('/:id/relations/:itemId')
        .get(function(req, res, next) {
            let call = sql + ' WHERE ' +
                'creature_has_relation.creature_id = ? AND ' +
                'creature_has_relation.relation_id = ?';
...
/** @deprecated */
module.exports.shields = function(router) {
    let table = 'shield',
        sql = 'SELECT ' +
            'shield.id, ' +
            'shield.name, ' +
    ...
    router.route('/:id/shields/:itemId')
        .get(function(req, res, next) {
            let call = sql + ' WHERE ' +
                'creature_has_' + table + '.creature_id = ? AND ' +
                'creature_has_' + table + '.' + table + '_id = ?';
    ...
    router.route('/:id/shields/:itemId/equip')
        .put(function(req, res, next) {
            equipment.equip(req.user, req.params.id, table, req.params.itemId, null, function(err) {
                if(err) return next(err);
    ...
    router.route('/:id/shields/:itemId/unequip')
        .put(function(req, res, next) {
            equipment.unequip(req.user, req.params.id, table, req.params.itemId, null, function(err) {
                if(err) return next(err);
...
module.exports.species = function(router) {
    ...
    router.route('/:id/species')
        ...
        .post(function(req, res, next) {
            relations.post(req, res, next, 'creature', req.params.id, table, req.body.insert_id, ['first']);
        });
    ...
    router.route('/:id/species/:itemId')
        ...
        .put(function(req, res, next) {
            relations.put(req, res, next, 'creature', req.params.id, table, req.params.itemId, ['first']);
        })
        .delete(function(req, res, next) {
            async.series([
                function(callback) {
                ...
                function(callback) {
                    query('SELECT first FROM creature_has_species WHERE creature_id = ? AND species_id = ? AND first = 1', [req.params.id, req.params.itemId], function(err, results) {
                        if(err) return callback(err);

                        if(results.length !== 0) return callback(new AppError(403, "Forbidden", "The primary species may not be removed from a creature.", "Removal of the primary species is impossible as it is too important to the character\'s creation and longevity."));
...
module.exports.spells = function(router) {
    ...
    router.route('/:id/spells')
        ...
        .post(function(req, res, next) {
            let creatureId = parseInt(req.params.id),
                spellId = parseInt(req.body.insert_id),
                cost;

            async.series([
                function(callback) {
                    query('SELECT cost FROM spell WHERE id = ?', [spellId], function(err, results) {
                        if(err) return callback(err);

                        cost = parseInt(results[0].cost);

                        callback();
                    });
                },
                function(callback) {
                    relation.post(req.user, 'creature', creatureId, table, spellId, req.body, null, callback);
                },
                function(callback) {
                    points.change(req.user, creatureId, table, -cost, callback);
                }
            ], function(err) {
                if(err) return next(err);
    ...
    router.route('/:id/spells/:itemId')
        ...
        .delete(function(req, res, next) {
            let creatureId = parseInt(req.params.id),
                spellId = parseInt(req.params.itemId),
                cost;
            ...
            async.series([
                ...
                function(callback) {
                    relation.remove(req.user, 'creature', creatureId, table, spellId, callback);
                },
                function(callback) {
                    points.change(req.user, creatureId, table, cost, callback);
                }
...
/** @deprecated */
module.exports.tactics = function(router) {
    let table = 'tactic',
        sql = 'SELECT ' +
            'tactic.id, ' +
            'tactic.name, ' +
    ...
    router.route('/:id/tactics/:itemId')
        .get(function(req, res, next) {
            let call = sql + ' WHERE ' +
                'creature_has_' + table + '.creature_id = ? AND ' +
                'creature_has_' + table + '.' + table + '_id = ?';
...
module.exports.weapons = function(router) {
    let table = 'weapon',
        valuesObject = {ignoreArray: ['weaponmod']},
        sql = 'SELECT ' +
            'weapon.id, ' +
            'weapon.name, ' +
            'weapon.description, ' +
    ...
    router.route('/:id/weapons/:itemId')
        ...
        .delete(function(req, res, next) {
            async.each([
                ...
                function(callback) {
                    equipment.unequip(req.user, req.params.id, table, req.params.itemId, {ignoreArray: ['weaponmod']}, callback);
                },
                function(callback) {
                    query('DELETE FROM creature_has_weapon WHERE creature_id = ? AND weapon_id = ?', [req.params.id, req.params.itemId], callback);
                },
                function(callback) {
                    query('DELETE FROM creature_has_weaponmod WHERE creature_id = ? AND weapon_id = ?', [req.params.id, req.params.itemId], callback);
                }
            ], function(err) {
                if(err) return next(err);
    ...
    router.route('/:id/weapons/:itemId/equip')
        .put(function(req, res, next) {
            equipment.equip(req.user, req.params.id, table, req.params.itemId, valuesObject, function(err) {
                if(err) return next(err);
    ...
    router.route('/:id/weapons/:itemId/unequip')
        .put(function(req, res, next) {
            equipment.unequip(req.user, req.params.id, table, req.params.itemId, valuesObject, function(err) {
                if(err) return next(err);
    ...
    router.route('/:id/weapons/:itemId/mods')
        ...
        .post(function(req, res, next) {
            ...
            async.series([
                ...
                function(callback) {
                    query('SELECT * FROM creature_has_weapon WHERE creature_id = ? AND weapon_id = ?', [creatureId, weaponId], function(err, results) {
                        if(err) return callback(err);

                        if(results.length === 0) return callback(new AppError(400, "Request Error", "Weapon not on creature", "The specified weapon is not in the creature list of weapon. Add it before you can add mods"));

                        callback();
                    });
                },
                function(callback) {
                    query('SELECT * FROM weapon_has_weaponmod WHERE weapon_id = ? AND weaponmod_id = ?', [weaponId, modId], function(err, results) {
                        if(err) return callback(err);

                        if(results.length === 0) return callback(new AppError(400, "Request Error", "Mod not associated with weapon", "The specified mod is not associated with that weapon. Did you send the correct insert_id?"));
    ...
    router.route('/:id/weapons/:itemId/mods/:modId')
        .delete(function(req, res, next) {
            ...
            async.each([
                ...
                function(callback) {
                    query('DELETE FROM creature_has_weaponmod WHERE creature_id = ? AND weapon_id = ? AND weaponmod_id = ?', [creatureId, weaponId, modId], callback);
                }
            ], function(err) {
                if(err) return next(err);
...
module.exports.corporation = function(router) {
    router.route('/:id/corporation')
        .get(function(req, res, next) {
            ...
            query(call, [req.params.id], function(err, results) {
                if(err) return next(err);
...
/** @deprecated */
module.exports.epoch = function(router) {
