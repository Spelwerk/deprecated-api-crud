...
let async = require('async'),
    ...
    database = require('../../app/initializers/database');
...
function relationValues(user, creatureId, tableName, relationName, additionId, subtractId, callback) {
    creatureId = parseInt(creatureId);
...
function relationValuesEach(user, creatureId, tableName, additionId, subtractId, options, callback) {
    options = options || {};

    let schema = database.getTable(tableName),
        schemaArray = schema.relations.creatures,
    ...
    async.each(relationArray, function(relationName, next) {
        relationValues(user, creatureId, tableName, relationName, additionId, subtractId, next);
    }, function(err) {
        callback(err);
    });
}
...
function equip(user, creatureId, tableName, tableId, shouldBeEquipped, options, callback) {
    creatureId = parseInt(creatureId);
    ...
    async.series([
        ...
        function(callback) {
            ...
            relationValuesEach(user, creatureId, tableName, addId, subId, options, callback);
        },
        function(callback) {
            if((isAlreadyEquipped && shouldBeEquipped) || (!isAlreadyEquipped && !shouldBeEquipped)) return callback();
...
function augmentationAdd(user, creatureId, bionicId, augmentationId, options, callback) {
    creatureId = parseInt(creatureId);
    ...
    async.series([
        function(callback) {
            query('SELECT augmentation_id FROM creature_has_augmentation WHERE creature_id = ? AND bionic_id = ? AND augmentation_id = ?', [creatureId, bionicId, augmentationId], function(err, results) {
                if(err) return callback(err);

                isAlreadyEquipped = results.length !== 0;

                callback();
            });
        },
        function(callback) {
            if(isAlreadyEquipped) return callback();
        ...
        function(callback) {
            ...
            relationValuesEach(user, creatureId, 'augmentation', augmentationId, null, options, callback);
        },
        function(callback) {
            if(isAlreadyEquipped) return callback();
...
function augmentationRemove(user, creatureId, bionicId, augmentationId, options, callback) {
    creatureId = parseInt(creatureId);
    bionicId = parseInt(bionicId);
    augmentationId = parseInt(augmentationId);
    options = options || {};
    ...
    async.series([
        ...
        function(callback) {
            relationValuesEach(user, creatureId, 'augmentation', null, augmentationId, options, callback);
        },
        function(callback) {
...
function bionicAdd(user, creatureId, tableName, tableId, callback) {
    creatureId = parseInt(creatureId);
    tableId = parseInt(tableId);
    ...
    async.series([
        ...
        function(callback) {
            ...
            for(let i in tableArray) {
                let existsInCreature = false;

                // loop through creature array
                for(let x in creatureArray) {

                    if(tableArray[i].id === creatureArray[x].id) {
                        existsInCreature = true;
                    }
                }

                if(!existsInCreature) bionicArray.push(parseInt(tableArray[i].id));
            }
        ...
        function(callback) {
            ...
            async.each(bionicArray, function(bionicId, next) {
                relationValuesEach(user, creatureId, 'bionic', bionicId, null, {ignoreArray: ['augmentation', 'software']}, next);
            }, function(err) {
                callback(err);
            });
        }
    ...
    async.series([
        ...
        function(callback) {
            query('SELECT weapon_id AS id FROM ' + weapon_is_table + ' WHERE ' + table_id + ' = ?', [tableId], function(err, results) {
                ...
                // Loop through results from table list and push the values into our new array
                for(let i in results) {
                    let id = parseInt(results[i].id);
...
module.exports.assets = function(router) {
    ...
    router.route('/:id/assets/:itemId')
        ...
        .delete(function(req, res, next) {
            async.each([
                ...
                function(callback) {
                    equip(req.user, req.params.id, table, req.params.itemId, false, null, callback);
                },
                function(callback) {
    ...
    router.route('/:id/assets/:itemId/equip/:equip')
        .put(function(req, res, next) {
            equip(req.user, req.params.id, table, req.params.itemId, req.params.equip, null, function(err) {
                if(err) return next(err);
    ...
    router.route('/:id/backgrounds')
        ...
        .post(function(req, res, next) {
            async.series([
                ...
                function(callback) {
                    relationValuesEach(req.user, req.params.id, table, req.body.insert_id, null, {ignoreArray: ['bionic', 'weapon']}, callback);
                },
                function(callback) {
                    bionicAdd(req.user, req.params.id, table, req.body.insert_id, callback);
                }
            ], function(err) {
                if(err) return next(err);
    ...
    router.route('/:id/backgrounds/:itemId')
        ...
        .delete(function(req, res, next) {
            async.series([
                ...
                function(callback) {
                    relationValuesEach(req.user, req.params.id, table, null, req.params.itemId, {ignoreArray: ['bionic', 'weapon']}, callback);
                },
                function(callback) {
                    query('DELETE FROM creature_has_' + table + ' WHERE creature_id = ? AND ' + table + '_id = ?', [req.params.id, req.params.itemId], callback);
                }
    ...
    router.route('/:id/bionics')
        ...
        .post(function(req, res, next) {
            async.series([
                ...
                function(callback) {
                    relationValuesEach(req.user, req.params.id, table, req.body.insert_id, null, {ignoreArray: ['augmentation', 'software']}, callback);
                }
            ], function(err) {
                if(err) return next(err);
    ...
    router.route('/:id/bionics/:itemId')
        ...
        .delete(function(req, res, next) {
            ...
            async.series([
                ...
                function(callback) {
                    relationValuesEach(req.user, creatureId, table, null, bionicId, {ignoreArray: ['augmentation', 'software']}, callback);
                },
                function(callback) {
                ...
                function(callback) {
                    query('SELECT augmentation_id AS id FROM creature_has_augmentation WHERE creature_id = ? AND bionic_id = ?', [creatureId, bionicId], function(err, results) {
                        ...
                        for(let i in results) {
                            augmentations.push(parseInt(results[i].id));
                        }
                ...
                function(callback) {
                    if(augmentations.length === 0) return callback();

                    async.each(augmentations, function(augmentationId, next) {
                        augmentationRemove(req.user, creatureId, bionicId, augmentationId, null, next);
                    }, function(err) {
                        callback(err);
                    });
                },
                function(callback) {
                    query('DELETE FROM creature_has_augmentation WHERE creature_id = ? AND bionic_id = ?', [creatureId, bionicId], callback);
                }
            ], function(err) {
                if(err) return next(err);
    ...
    router.route('/:id/bionics/:itemId/augmentations')
        ...
        .post(function(req, res, next) {
            ...
            async.series([
                ...
                function(callback) {
                    augmentationAdd(req.user, creatureId, bionicId, augmentationId, {ignoreArray: ['software']}, callback);
                }
            ], function(err) {
                if(err) return next(err);
    ...
    router.route('/:id/bionics/:itemId/augmentations/:augmentationId')
        .delete(function(req, res, next) {
            ...
            async.series([
                ...
                function(callback) {
                    augmentationRemove(req.user, creatureId, bionicId, augmentationId, {ignoreArray: ['software']}, callback);
                },
                function(callback) {
                    query('DELETE FROM creature_has_augmentation WHERE creature_id = ? AND bionic_id = ? AND augmentation_id = ?', [creatureId, bionicId, augmentationId], callback);
                }
    ...
    router.route('/:id/gifts')
        ...
        .post(function(req, res, next) {
            async.series([
                ...
                function(callback) {
                    relationValuesEach(req.user, req.params.id, table, req.body.insert_id, null, null, callback);
                }
            ], function(err) {
                if(err) return next(err);
    ...
    router.route('/:id/gifts/:itemId')
        ...
        .delete(function(req, res, next) {
            async.series([
                ...
                function(callback) {
                    relationValuesEach(req.user, req.params.id, table, null, req.params.itemId, null, callback);
                },
                function(callback) {
                    query('DELETE FROM creature_has_' + table + ' WHERE creature_id = ? AND ' + table + '_id = ?', [req.params.id, req.params.itemId], callback);
                }
    ...
    router.route('/:id/manifestations')
        ...
        .post(function(req, res, next) {
            ...
            async.series([
                ...
                function(callback) {
                    weaponAdd(req.user, creatureId, 'manifestation', manifestationId, callback);
                }
            ], function(err) {
                if(err) return next(err);
    ...
    router.route('/:id/manifestations/:itemId')
        ...
        .delete(function(req, res, next) {
            ...
            async.series([
                ...
                function(callback) {
                    weaponRemove(req.user, creatureId, 'manifestation', manifestationId, callback);
                }
            ], function(err) {
                if(err) return next(err);
    ...
    router.route('/:id/milestones')
        ...
        .post(function(req, res, next) {
            ...
            async.series([
                ...
                function(callback) {
                    relationValuesEach(req.user, creatureId, table, milestoneId, null, {ignoreArray: ['bionic', 'loyalty', 'weapon']}, callback);
                },
                function(callback) {
                    bionicAdd(req.user, req.params.id, table, req.body.insert_id, callback);
                },
    ...
    router.route('/:id/milestones/:itemId')
        ...
        .delete(function(req, res, next) {
            async.series([
                ...
                function(callback) {
                    relationValuesEach(req.user, req.params.id, table, null, req.params.itemId, {ignoreArray: ['bionic', 'loyalty']}, callback);
                },
                function(callback) {
                    query('DELETE FROM creature_has_' + table + ' WHERE creature_id = ? AND ' + table + '_id = ?', [req.params.id, req.params.itemId], callback);
                }
...
module.exports.shields = function(router) {
    ...
    router.route('/:id/shields/:itemId/equip/:equip')
        .put(function(req, res, next) {
            equip(req.user, req.params.id, table, req.params.itemId, req.params.equip, null, function(err) {
                if(err) return next(err);
...
module.exports.weapons = function(router) {
    ...
    router.route('/:id/weapons/:itemId')
        ...
        .delete(function(req, res, next) {
            async.each([
                ...
                function(callback) {
                    equip(req.user, req.params.id, table, req.params.itemId, false, {ignoreArray: ['weaponmod']}, callback);
                },
                function(callback) {
    ...
    router.route('/:id/weapons/:itemId/equip/:equip')
        .put(function(req, res, next) {
            equip(req.user, req.params.id, table, req.params.itemId, req.params.equip, {ignoreArray: ['weaponmod']}, function(err) {
                if(err) return next(err);
