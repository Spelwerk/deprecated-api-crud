            ...
            async.series([
                ...
                function(callback) {
                    query('SELECT bionic_id FROM creature_has_bionic WHERE creature_id = ? AND bionic_id = ?', [id, bionicId], function(err, results) {
                        ...
                        if(!results[0]) return callback(Err.CustomError(400, 'Request Error', 'Bionic is not equipped on creature', 'The bionic supplied in the request does not exist on the creature.'));
                ...
                function(callback) {
                    query('SELECT augmentation_id FROM bionic_has_augmentation WHERE bionic_id = ? AND augmentation_id = ?', [bionicId, augmentationId], function(err, results) {
                        ...
                        if(!results[0]) return callback(Err.CustomError(400, 'Request Error', 'Augmentation not associated with bionic', 'The specified augmentation is not associated with that bionic.'));
            ...
            async.series([
                ...
                function(callback) {
                    query('SELECT first FROM creature_has_species WHERE creature_id = ? AND species_id = ?', [req.params.id, req.params.itemId], function(err, results) {
                        ...
                        if(!results[0].first) return callback(Err.CustomError(403, 'Forbidden', 'The primary species may not be removed from a creature.', 'Removal of the primary species is impossible as it is too important to the character\'s creation and longevity.'));
            ...
            async.series([
                ...
                function(callback) {
                    query('SELECT weapon_id FROM creature_has_weapon WHERE creature_id = ? AND weapon_id = ?', [creatureId, weaponId], function(err, results) {
                        ...
                        if(!results[0]) return callback(Err.CustomError(400, 'Request Error', 'Weapon not on creature', 'The specified weapon is not in the creature list of weapons. Add it before you can add mods\''));
                ...
                function(callback) {
                    query('SELECT weaponmod_id FROM weapon_has_weaponmod WHERE weapon_id = ? AND weaponmod_id = ?', [weaponId, modId], function(err, results) {
                        ...
                        if(!results[0]) return callback(Err.CustomError(400, 'Request Error', 'Mod not associated with weapon', 'The specified mod is not associated with that weapon. Did you send the correct insert_id?'));
