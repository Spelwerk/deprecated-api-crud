...
function relationValuesEach(user, creatureId, tableName, relationArray, additionId, subtractId, callback) {
...
function activate(user, creatureId, bionicId, augmentationId, relationArray, boolean, callback) {
    ...
    async.series([
        ...
        function(callback) {
            if((equipped && boolean) || (!equipped && !boolean) || !boolean) return callback();
...
module.exports.backgrounds = function(router) {
    let table = 'background',
        backgroundArray = ['asset', 'attribute', 'primal', 'skill', 'weapon'],
        bionicArray = ['attribute', 'skill'],
        sql = 'SELECT ' +
            'background.id, ' +
            'background.name, ' +
    ...
    router.route('/:id/backgrounds')
        ...
        .post(function(req, res, next) {
            ...
            async.series([
                ...
                function(callback) {
                    relationValuesEach(req.user, creatureId, table, backgroundArray, backgroundId, null, callback);
                },
                function(callback) {
                    query('SELECT bionic_id AS id FROM background_has_bionic WHERE background_id = ?', [backgroundId], function(err, results) {
                        if(err) return callback(err);

                        if(results.length === 0) return callback();

                        for(let i in results) {
                            bionics.push(parseInt(results[i].id));
                        }
                ...
                function(callback) {
                    if(bionics.length === 0) return callback();

                    let call = 'INSERT INTO creature_has_bionic (creature_id,bionic_id) VALUES ';

                    for(let i in bionics) {
                        call += '(' + creatureId + ',' + bionics[i] + '),';
                    }

                    call = call.slice(0, -1);

                    query(call, null, next);
                },
                function(callback) {
                    if(bionics.length === 0) return callback();

                    async.each(bionics, function(bionicId, next) {
                        relationValuesEach(req.user, creatureId, 'bionic', bionicArray, bionicId, null, next);
                    }, function(err) {
                        callback(err);
                    });
                }
    ...
    router.route('/:id/backgrounds/:itemId')
        ...
        .delete(function(req, res, next) {
            async.series([
                ...
                function(callback) {
                    relationValuesEach(req.user, req.params.id, table, backgroundArray, null, req.params.itemId, callback);
                },
                function(callback) {
                    query('DELETE FROM creature_has_' + table + ' WHERE creature_id = ? AND ' + table + '_id = ?', [req.params.id, req.params.itemId], callback);
                }
...
module.exports.bionics = function(router) {
    let table = 'bionic',
        bionicArray = ['attribute', 'skill'],
        augmentationArray = ['attribute', 'expertise', 'skill'],
        sql = 'SELECT ' +
            'bionic.id, ' +
            'bionic.name, ' +
            'bionic.description, ' +
    ...
    router.route('/:id/bionics')
        ...
        .post(function(req, res, next) {
            async.series([
                ...
                function(callback) {
                    relationValuesEach(req.user, req.params.id, table, bionicArray, req.body.insert_id, null, callback);
                }
            ], function(err) {
                if(err) return next(err);
    ...
    router.route('/:id/bionics/:itemId')
        ...
        .delete(function(req, res, next) {
            ...
            async.series([
                ...
                function(callback) {
                    relationValuesEach(req.user, creatureId, table, bionicArray, null, bionicId, callback);
                },
                function(callback) {
                ...
                function(callback) {
                    ...
                    async.each(augmentations, function(augmentationId, next) {
                        activate(req.user, creatureId, bionicId, augmentationId, augmentationArray, false, next);
                    }, function(err) {
                        callback(err);
                    });
                },
    ...
    router.route('/:id/bionics/:itemId/augmentations')
        ...
        .post(function(req, res, next) {
            ...
            async.series([
                ...
                function(callback) {
                    activate(req.user, creatureId, bionicId, augmentationId, augmentationArray, true, callback);
                }
            ], function(err) {
                if(err) return next(err);
    ...
    router.route('/:id/bionics/:itemId/augmentations/:augmentationId')
        .delete(function(req, res, next) {
            ...
            async.series([
                ...
                function(callback) {
                    activate(req.user, creatureId, bionicId, augmentationId, augmentationArray, false, callback);
                },
                function(callback) {
...
module.exports.milestones = function(router) {
    let table = 'milestone',
        milestoneArray = ['asset', 'attribute', 'primal', 'skill', 'weapon'],
        bionicArray = ['attribute', 'skill'],
        sql = 'SELECT ' +
            'milestone.id, ' +
            'milestone.name, ' +
    ...
    router.route('/:id/milestones')
        ...
        .post(function(req, res, next) {
            ...
            async.series([
                ...
                function(callback) {
                    relationValuesEach(req.user, creatureId, table, milestoneArray, milestoneId, null, callback);
                },
                function(callback) {
                    query('SELECT bionic_id AS id FROM milestone_has_bionic WHERE background_id = ?', [milestoneId], function(err, results) {
                        if(err) return callback(err);
                ...
                function(callback) {
                    ...
                    async.each(bionics, function(bionicId, next) {
                        relationValuesEach(req.user, creatureId, 'bionic', bionics, bionicId, null, next);
                    }, function(err) {
                        callback(err);
                    });
                },
                function(callback) {
                    query('SELECT * FROM milestone_has_loyalty WHERE milestone_id = ?', [milestoneId], function(err, results) {
                        ...
                        for(let i in results) {
                            ...
                            loyalties.push({loyaltyId: lId, wealthId: wId, occupation: wOc});
                        }
                ...
                function(callback) {
                    if(loyalties.length === 0) return callback();

                    async.each(loyalties, function(object, next) {
    ...
    router.route('/:id/milestones/:itemId')
        ...
        .delete(function(req, res, next) {
            async.series([
                ...
                function(callback) {
                    relationValuesEach(req.user, req.params.id, table, milestoneArray, null, req.params.itemId, callback);
                },
                function(callback) {
                    query('DELETE FROM creature_has_' + table + ' WHERE creature_id = ? AND ' + table + '_id = ?', [req.params.id, req.params.itemId], callback);
                }
