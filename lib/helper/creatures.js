...
function getIdAndValue(tableName, tableId, relationName, callback) {
    ...
    query('SELECT ' + relation_id + ' AS id, value FROM ' + table_has_relation + ' WHERE ' + table_id + ' = ?', [tableId], function(err, results) {
        if(err) return callback(err);

        if(results.length === 0) return callback(null, array);
...
function post(user, body, callback) {
    ...
    let worldAttributeArray = [],
        speciesAttributeArray = [],
        attributeArray = [];

    let worldSkillArray = [],
        speciesSkillArray = [];

    let worldId = parseInt(body.world_id),
        speciesId = parseInt(body.species_id),
    ...
    async.series([
        ...
        // Creating Species
        function(callback) {
            query('INSERT INTO creature_has_species (creature_id,species_id,first) VALUES (?,?,1)', [creatureId, speciesId], callback);
        },

        // Adds weapon from species with weapon_is_species
        function(callback) {
            exclusive.addFromCombination(user, creatureId, 'species', speciesId, 'weapon', true, callback);
        },

        // Attributes
        function(callback) {
            getIdAndValue('world', worldId, 'attribute', function(err, array) {
                if(err) return callback(err);

                worldAttributeArray = array;

                callback();
            });
        },
        function(callback) {
            getIdAndValue('species', worldId, 'attribute', function(err, array) {
                if(err) return callback(err);

                speciesAttributeArray = array;

                callback();
            });
        },
        function(callback) {
            if(worldAttributeArray.length === 0 && speciesAttributeArray === 0) return callback();

            attributeArray = worldAttributeArray;

            for(let i in attributeArray) {
                for(let x in speciesAttributeArray) {
                    if(attributeArray[i].id !== speciesAttributeArray[x].id) continue;

                    attributeArray[i].value += speciesAttributeArray[x].value;
                }
            }
        ...
        // Skills
        function(callback) {
            exclusive.getTableArray('world_has_skill', 'world', worldId, 'skill', function(err, array) {
                if(err) return callback(err);

                worldSkillArray = array;

                callback();
            });
        },
        function(callback) {
            exclusive.getTableArray('skill_is_species', 'species', speciesId, 'skill', function(err, array) {
                if(err) return callback(err);

                speciesSkillArray = array;
        ...
        function(callback) {
            ...
            for(let x in worldSkillArray) {
                sql += '(' + creatureId + ',' + worldSkillArray[x] + '),';
            }

            for(let x in speciesSkillArray) {
                sql += '(' + creatureId + ',' + speciesSkillArray[x] + '),';
            }
...
module.exports.loyalties = function(router) {
    ...
    router.route('/:id/loyalties/:uqId/create')
        .post(function(req, res, next) {
            ...
            let creature = {firstname: null, lastname: null, world_id: null, species_id: speciesId, wealth_id: null, occupation: null},
            ...
            async.series([
                ...
                function(callback) {
                    query('SELECT world_id AS id FROM creature WHERE id = ?', [creatureId], function(err, results) {
                        if(err) return callback(err);

                        if(results.length === 0) return callback(new DatabaseRowNotFoundError);

                        creature.world_id = parseInt(results[0].id);
