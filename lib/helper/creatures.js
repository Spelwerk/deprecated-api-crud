...
function post(user, body, callback) {
    ...
    async.series([
        ...
        function(callback) {
            let experienceId = defaults.attribute.experience;

            let age = parseFloor(body.age, 1),
                multiplier = parseFloor(species.multiply_points, 1),
                maximum = parseFloor(world.max_points, 1);

            let multiplied = age * multiplier;

            let points = multiplied > maximum
                ? maximum
                : multiplied;

            value.change(user, creatureId, 'attribute', experienceId, points, callback);
        },
            ...
            async.series([
                ...
                function(callback) {
                    query('SELECT * FROM creature_has_loyalty WHERE id = ?', [uqId], function(err, results) {
                        ...
                        creature.creaturetype_id = parseInt(req.body.creaturetype_id) || defaults.creatureType.avatar;
