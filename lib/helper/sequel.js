...
let AppError = require('../errors/app-error'),
    UserNotAdministratorError = require('../errors/user-not-administrator-error');

let query = require('../sql/query'),
    elemental = require('../sql/elemental');
...
module.exports.put = function(req, res, next, tableName, tableId) {
    elemental.put(req.user, req.body, tableName, tableId, function(err) {
        if(err) return next(err);
...
module.exports.delete = function(req, res, next, tableName, tableId) {
    elemental.remove(req.user, tableName, tableId, function(err) {
        if(err) return next(err);
...
module.exports.clone = function(req, res, next, tableName, tableId) {
    elemental.clone(req.user, tableName, tableId, function(err, id) {
        if(err) return next(err);
